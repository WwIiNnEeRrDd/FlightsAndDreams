...
$base_path = 'FlightsAndDreams'; // Ajusta si tu proyecto está en otra carpeta
$index = array_search($base_path, $request_uri);

// Define controlador, acción y parámetros adicionales
$controller = $request_uri[$index + 1] ?? null;
$action = $request_uri[$index + 2] ?? null;

$estados = ['pendiente', 'confirmado', 'cancelado'];

// Verificar si el tercer parámetro es un servicio o un id_paquete
$third_param = $request_uri[$index + 3] ?? null;
